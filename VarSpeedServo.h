...
public:
  VarSpeedServo();
  uint8_t attach(int pin, int initial_pos = DEFAULT_PULSE_WIDTH);           // attach the given pin to the next free channel, sets pinMode, returns channel number or 0 if failure
  uint8_t attach(int pin, int min, int max, int initial_pos = DEFAULT_PULSE_WIDTH); // as above but also sets min and max values for writes. 
  void detach();
